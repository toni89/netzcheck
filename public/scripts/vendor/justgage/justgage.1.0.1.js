...
JustGage = function(config) { 
  ...
  // title
  var titleFontSize = 18;//CHANGE: ((widgetH / 8) > 10) ? (widgetH / 10) : 10;
  var titleX = dx + widgetW / 2;
  var titleY = dy + widgetH / 8; //CHANGE: / 6.5
  ...
  // value
  var myValue = (this.originalValue==0) ? '-' : this.originalValue;
  this.txtValue = this.canvas.text(this.params.valueX, this.params.valueY, myValue + ' / ' + this.config.max);
  this.txtValue. attr({
...
JustGage.prototype.refresh = function(val) {
  ...
  this.canvas.getById(this.config.id+"-txtvalue").attr({"text":originalVal + ' / ' + this.config.max});
